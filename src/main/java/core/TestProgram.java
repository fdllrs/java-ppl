...
public class TestProgram {
	...
	public static List<Expression> Ejemplo7() {
		// Equivalent to Ejemplo2 (bits sum) but using the Binomial distribution directly
		String program = "(let [total (sample (binomial 8 0.5))] " +
						 "  (observe (normal 7 2) total) total)";

		return Parser.parse(program);
	}

	public static List<Expression> Ejemplo8() {
	...
	public static List<Expression> Ejemplo9() {
		// Uniform-Normal model
		String program = "(let [x (sample (uniform 0.0 10.0))] " +
						 "  (observe (normal x 1.0) 4.5) x)";

		return Parser.parse(program);
	}

	public static List<Expression> Ejemplo10() {
		// Beta-Bernoulli conjugate model
		String program = "(let [p (sample (beta 2.0 2.0))] " +
						 "  (observe (bernoulli p) 1.0) p)";

		return Parser.parse(program);
	}

	public static List<Expression> Ejemplo11() {
