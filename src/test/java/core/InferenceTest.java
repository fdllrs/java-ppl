...
public class InferenceTest {
	...
	public void testSoftmaxThrowsOnAllNegativeInfinity() {
		ArrayList<Double> allNegInf = new ArrayList<>(List.of(Double.NEGATIVE_INFINITY,
	...
	public void testLWStdDevConverges() {
		List<Expression> program = TestProgram.normalNormalConjugate();
		Posterior<Double> p =
				new LikelihoodWeighting<Double>(program, new Random(42), 50000).run();
	...
	public void testLWEffectiveSampleSizePositive() {
		List<Expression> program = TestProgram.normalNormalConjugate();
		Posterior<Double> p =
				new LikelihoodWeighting<Double>(program, new Random(42), 10000).run();
	...
	public void testSSMHEssEqualsNSamples() {
		...
		List<Expression> program = TestProgram.normalNormalConjugate();
		int iterations = 1000;
		Posterior<Double> p = new SSMetropolisHastings<Double>(program,
															   new Random(42),
															   100,
															   iterations).run();
	...
	public void testSMCEssEqualsParticleCount() {
		...
		List<Expression> program = TestProgram.normalNormalConjugate();
		int particles = 500;
		Posterior<Double> p = new SequentialMonteCarlo<Double>(program,
															   new Random(42),
															   particles).run();
